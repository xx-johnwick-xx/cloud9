...
import { ChatGPTConversation } from './cloud9-conversation'
...
export class Cloud9AI {
   ...
   * @param apiBaseUrl - Optional override; the base URL for Cloud9 webapp's API (`/api`)
   * @param backendApiBaseUrl - Optional override; the base URL for the Cloud9 backend API (`/backend-api`)
   * @param userAgent - Optional override; the `user-agent` header to use with Cloud9 requests
  ...
  constructor(opts: {
    ...
    if (!this._sessionToken) {
      throw new types.Cloud9Error('Cloud9 invalid session token')
    }

    if (!this._clearanceToken) {
      throw new types.Cloud9Error('Cloud9 invalid clearance token')
    }
  }
   ...
   * @param opts.action - Optional Cloud9 `action` (either `next` or `variant`)
  ...
  async refreshAccessToken(): Promise<string> {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        if (!r.ok) {
          const error = new types.Cloud9Error(`${r.status} ${r.statusText}`)
          error.response = r
      ...
      if (!accessToken) {
        const error = new types.Cloud9Error('Unauthorized')
      ...
      if (appError) {
        if (appError === 'RefreshAccessTokenError') {
          const error = new types.Cloud9Error('session token may have expired')
          error.response = response
          error.statusCode = response?.status
          error.statusText = response?.statusText
          throw error
        } else {
          const error = new types.Cloud9Error(appError)
          error.response = response
          error.statusCode = response?.status
          error.statusText = response?.statusText
          throw error
      ...
      const error = new types.Cloud9Error(
        `Cloud9 failed to refresh auth token. ${err.toString()}`
      )
      error.response = response
      error.statusCode = response?.status
      error.statusText = response?.statusText
